...
class MilestoneTransactionCompletedBuyerNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'instantescrow') {
            ...
            return (new MailMessage)
                ...
                ->view('emails.instantescrow.milestone_completed_buyer', [
                    'user' => $notifiable,
        ...
        if (!empty($this->payload->business)) {
            switch ($this->payload->business->business_type) {
                ...
                case "marketplace":
                    $mail->view('emails.transactions.milestone_transaction_completed', ['user' => $notifiable, 'payload' => $this->payload, 'links' => $this->links])
                        ->subject($this->subject);
                    break;

                case "social_commerce":
                    $mail->view('emails.social_commerce.milestone_confirm_completion', ['user' => $notifiable, 'payload' => $this->payload, 'links' => $this->links])
                        ->subject($this->subject);
                    break;
