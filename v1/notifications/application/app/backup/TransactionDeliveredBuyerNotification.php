...
class TransactionDeliveredBuyerNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'instantescrow') {
            ...
            return (new MailMessage)
                ...
                ->view('emails.instantescrow.delivered_buyer', [
                    'user' => $notifiable,
        ...
        if (!empty($this->payload->business)) {
            switch ($this->payload->business->business_type) {
                ...
                case "marketplace":
                    $mail->view('emails.transactions.transaction_delivered', ['user' => $notifiable, 'payload' => $this->payload, 'links' => $this->links])
                        ->subject($this->subject);
                    break;

                case "social_commerce":
                    $mail->view('emails.social_commerce.confirm_delivery', ['user' => $notifiable, 'payload' => $this->payload, 'links' => $this->links])
                        ->subject($this->subject);
                    break;
