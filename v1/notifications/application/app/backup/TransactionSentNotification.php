...
class TransactionSentNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'instantescrow') {
            return $mail->view('emails.instantescrow.transaction_sent', $data)
                ->subject('Your new escrow transaction was created and sent successfully.')
                ->attachData($pdf->output(), "transaction_sent.pdf");
        }

        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'transfer') {
            $pdf = PDF::loadView('emails.transfer.transaction_sent', $data);
            return $mail->view('emails.transfer.transaction_sent', $data)
                ->subject('Your funds transfer request was created successfully.')
                ->attachData($pdf->output(), "transaction_sent.pdf");
        }
        ...
        if (!empty($this->payload->business)) {
            switch ($this->payload->business->business_type) {
                case "marketplace":
                    $mail->view('emails.marketplace.transaction_sent', $data)
                        ->subject('Your new escrow transaction was created and sent successfully')
                        ->attachData($pdf->output(), "transaction_sent.pdf");
                    break;

                case "social_commerce":
                    $mail->view('emails.transactions.transaction_sent', $data)
                        ->subject('Your new escrow transaction was created and sent successfully')
                        ->attachData($pdf->output(), "transaction_sent.pdf");
                    break;
