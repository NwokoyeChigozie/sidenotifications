...
class TransactionReceivedBuyerNotification extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
        ...
        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'instantescrow') {
            ...
            return $this->view('emails.instantescrow.transaction_received_buyer')->with($data)
        ...
        if (!empty($this->payload->business)) {
            switch ($this->payload->business->business_type) {
                case "marketplace":
                    ...
                    $mail = $this->view('emails.marketplace.transaction_received_buyer')->with($data)
                ...
                case "social_commerce":
                    ...
                    $mail = $this->view('emails.transactions.transaction_received_buyer')->with($data)
                        ->subject('You have a new escrow transaction')->attachData($pdf->output(), "transaction_received.pdf", [
                            'mime' => 'application/pdf',
                        ]);
