...
class TransactionReceivedNotification extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
        ...
        if (isset($this->payload->transaction->source) && $this->payload->transaction->source == 'instantescrow') {
            $pdf = PDF::loadView('emails.instantescrow.transaction_received', $data);
            return $this->view('emails.instantescrow.transaction_received')->with($data)
        ...
        if (!empty($this->payload->business)) {
            switch ($this->payload->business->business_type) {
                case "marketplace":
                    $pdf = PDF::loadView('emails.marketplace.transaction_received', $data);
                    $mail = $this->view('emails.marketplace.transaction_received')->with($data)
                ...
                case "social_commerce":
                    ...
                    $mail = $this->view('emails.transactions.transaction_received')->with($data)
                        ->subject('You have a new escrow transaction')->attachData($pdf->output(), "transaction_received.pdf", [
                            'mime' => 'application/pdf',
                        ]);
